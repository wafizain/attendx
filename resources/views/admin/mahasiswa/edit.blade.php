...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <form id="mahasiswaEditForm" action="{{ route('mahasiswa.update', $mahasiswa->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" name="nim" class="form-control text-uppercase @error('nim') is-invalid @enderror" 
                                   value="{{ old('nim', $mahasiswa->nim) }}" required style="font-family: monospace;">
                            @error('nim')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_prodi" class="form-select @error('id_prodi') is-invalid @enderror" required>
                                ...
                                @foreach($prodiList as $prodi)
                                    <option value="{{ $prodi->id }}" {{ old('id_prodi', $mahasiswa->id_prodi) == $prodi->id ? 'selected' : '' }}>
                                        {{ $prodi->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_kelas" class="form-select @error('id_kelas') is-invalid @enderror">
                                ...
                                @foreach($kelasList as $kelas)
                                    <option value="{{ $kelas->id }}" {{ old('id_kelas', $mahasiswa->id_kelas) == $kelas->id ? 'selected' : '' }}>
                            ...
                            @error('id_kelas')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">Jika kelas diubah, mahasiswa akan dipindahkan ke kelas yang dipilih</small>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="number" name="angkatan" class="form-control @error('angkatan') is-invalid @enderror" 
                                   value="{{ old('angkatan', $mahasiswa->angkatan) }}" min="2000" max="{{ date('Y') + 1 }}" required>
                            @error('angkatan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="alert alert-warning">
                    ...
                    <strong>Catatan:</strong> Data login (username dan password) tidak dapat diubah di halaman ini. Gunakan tombol Reset Username/Password di halaman detail mahasiswa.
                </div>

                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('mahasiswa.index') }}" class="btn btn-secondary" style="background-color:#6C757D;border-color:#6C757D;">Batal</a>
...
document.addEventListener('DOMContentLoaded', function() {
  const form = document.getElementById('mahasiswaEditForm');
  if (!form) return;
