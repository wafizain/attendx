                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select name="akreditasi" class="form-select @error('akreditasi') is-invalid @enderror">
                                ...
                                <option value="Baik" {{ old('akreditasi', $prodi->akreditasi) == 'Baik' ? 'selected' : '' }}>Baik</option>
                                <option value="Baik Sekali" {{ old('akreditasi', $prodi->akreditasi) == 'Baik Sekali' ? 'selected' : '' }}>Baik Sekali</option>
                                <option value="Unggul" {{ old('akreditasi', $prodi->akreditasi) == 'Unggul' ? 'selected' : '' }}>Unggul</option>
                            </select>
                            @error('akreditasi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                <option value="1" {{ old('status', $prodi->status) == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('status', $prodi->status) == '0' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="kaprodi_user_id" class="form-select @error('kaprodi_user_id') is-invalid @enderror">
                                ...
                                @foreach(\App\Models\User::where('role', 'dosen')->where('status', 'aktif')->get() as $dosen)
                                    <option value="{{ $dosen->id }}" {{ old('kaprodi_user_id', $prodi->kaprodi_user_id) == $dosen->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12">
                        <div class="mb-3">
                            ...
                            <textarea name="deskripsi" rows="4" class="form-control @error('deskripsi') is-invalid @enderror" 
                                      placeholder="Deskripsi singkat tentang program studi...">{{ old('deskripsi', $prodi->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
